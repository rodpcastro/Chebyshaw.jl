# Chebyshaw
`ChebyshevSeries` is the main type of the `Chebyshaw` module and represents a n-dimensional Chebyshev series, defined by its coefficients and domain boundaries. The following expressions provide an overview of the algorithm used for its evaluation, using the one-dimensional case as a concrete example.
...
With the coefficients $a_k$ at hand[^1], the *Clenshaw* (1955) algorithm is the method used to obtain the value of $s_n$. The algorithm makes use of the following recurrence
...
[^1]: To compute Chebyshev coefficients, the package [FastChebInterp.jl] is recommended.
